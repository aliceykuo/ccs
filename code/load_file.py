...
class ImageLoader(object):
    ...
    def dom_colors_by_segment(self, k=10):
        self.seg_dom_colors = None 
        for segment_set in self.all_segmented_filenames:
            print segment_set
            dom_colors =[] 
            #dominant colors for each segment 
            for segment in segment_set:
                ...
                for i in codes[index_top]:
                    ...
                    # print 'most frequent is %s (#%s)' % (i, colour)
                dom_colors.append(img_dom_colors)
            # print dom_colors #list of lists 
            for dom_col in dom_colors:
                dom_col = np.array(dom_col).ravel()[np.newaxis, :]
            if self.seg_dom_colors is None:
                self.seg_dom_colors = dom_col
            else: 
                self.seg_dom_colors = np.concatenate((self.seg_dom_colors, dom_col),  axis=0)
    ...
    def transform_filter(self, img_filter = 'hist'):
        ...
        for i, pair in enumerate(self.img_label_pair):
            ...
            filename = trans_dir + str(label_int) + '_'+str(i)+'.jpg'
    ...
    def post_trans(self):
        ...
        segment_colors = self.segment_image()
        print "completed dominant colors by segment"
        sift_mat = self.sift()
        # dom_colors = self.dom_colors(files='transformed')
        print "segment colors", type(segment_colors)
        self.post_feat_mat = None 
        for seg_color in segment_colors:
            seg_color = np.array(seg_color).ravel()[np.newaxis, :]
            if self.post_feat_mat is None:
                self.post_feat_mat = seg_color
            else: 
                self.post_feat_mat = np.concatenate((self.post_feat_mat,seg_color),  axis=0)
    ...
    def save_pkl(self, size=28, samples='1' ,scaled = 'scaled', colors='k5'):
        ...
        pkl_path = '../images/pkl/mat' + '_' + str(size) + '_'+ samples + '_'+ scaled  + '_' + colors + '_nonwhiten_color_allwsift.pkl'
...
if __name__ == '__main__':
   ...
   loader.save_pkl(size=30, samples='test', scaled='stdscaled', colors='k5')
